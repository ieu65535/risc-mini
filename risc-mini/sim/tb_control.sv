...
module tb_control();
    ...
    task run_and_check(
        ...
        logic pass;
    begin
        // Give the combinational ROM model one timestep to observe the program
        // loaded while reset is asserted before the first fetch is accepted.
        #1;
        rst = 0;
        // 等待足够多的周期让控制流跑完
...
`ifdef DUMP_WAVES
    initial begin
        ...
        $dumpvars(0, tb_control.dut.u_fetch);
    end
`endif
