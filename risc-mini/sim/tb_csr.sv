...
`endif
        ...
        inst_mem[10] = 32'h34102673; // 28: csrr x12, mepc    -> mepc按4字节对齐，预期0x344
        ...
        if (dut.u_reg_file.regs[9] === 32'h1808) $display("[PASS] CSRRS(置位): x9 = 0x1808");
        else begin $display("[FAIL] CSRRS(置位): x9 = 0x%h, 期望 0x1808", dut.u_reg_file.regs[9]); error_count = error_count + 1; end

        if (dut.u_reg_file.regs[10] === 32'h1800) $display("[PASS] CSRRC(清零): x10 = 0x1800");
        else begin $display("[FAIL] CSRRC(清零): x10 = 0x%h, 期望 0x1800", dut.u_reg_file.regs[10]); error_count = error_count + 1; end

        // --- 进阶测试1检查 ---
        if (dut.u_reg_file.regs[12] === 32'h344) $display("[PASS] ALU->CSR前递及mepc对齐: x12 = 0x344");
        else begin $display("[FAIL] ALU->CSR前递及mepc对齐: x12 = 0x%h, 期望 0x344", dut.u_reg_file.regs[12]); error_count = error_count + 1; end
